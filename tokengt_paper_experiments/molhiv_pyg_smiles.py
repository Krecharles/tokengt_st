...
class PygGraphPropPredDatasetWithSmiles(InMemoryDataset):
    def __init__(self, name, root = 'dataset', transform=None, pre_transform = None, meta_dict = None):
        ...
        if meta_dict is None:
            ...
            ogb_dir = os.path.dirname(ogb.graphproppred.dataset_pyg.__file__)
            master = pd.read_csv(os.path.join(ogb_dir, 'master.csv'), index_col=0, keep_default_na=False)
        ...
        super(PygGraphPropPredDatasetWithSmiles, self).__init__(self.root, transform, pre_transform)
    ...
    def process(self):
        ...
        smiles = None
        mapping_path = osp.join(self.root, 'mapping', 'mol.csv.gz')
        df_map = pd.read_csv(mapping_path)
        smiles = df_map['smiles'].tolist()
        assert len(smiles) == len(data_list), \
            f"SMILES length ({len(smiles)}) != number of graphs ({len(data_list)})"
        ...
        if self.pre_transform is not None:
            data_list = [self.pre_transform(data) for data in tqdm(data_list, desc="Pre-transforming data")]
...
if __name__ == '__main__':
    ...
    pyg_dataset = PygGraphPropPredDatasetWithSmiles(name = 'ogbg-molhiv')
    print(pyg_dataset.num_classes)
    split_index = pyg_dataset.get_idx_split()
    print(pyg_dataset[0])
